...
RSS <- function(parameters) {
  ...
  w1 = 4
  w2 = 1
  w1*sum((log(Active) - log(fit1))^2) + w2*sum((log(Removed) - log(fit2))^2)  # weighted
...
# start_date  <- "2020-03-18"
# end_date    <- "2020-04-07"  # very high R2, but lower susceptibles

# MCO week 2 -> Today # to take into account MCO effect 1 week
...
parameters_values_lower <- c(1/100, 1/19.5)  # days recover 6 weeks, reduce bound bcs not many severe, oberved show quick recovery
parameters_values_upper <- c(1, 1/11)  # updated to min 11 days

# Total population

# function
# find_N = function(N, inits, Day, parameters_values, parameters_values_lower, parameters_values_upper) {
#   t = 1:max(Day)
...
#   N[Opt_min_loc]
# }  find_N(N, inits, Day, parameters_values, parameters_values_lower, parameters_values_upper) # not working, ode looks to outside variables
...
# --- selected min location of susceptible ---
...
fitplot1 = ggplot(fitted_projected, aes(x = Date)) + geom_line(aes(y = I, color = "Infectious")) + 
  ...
  labs(y = "Infectious", title = paste("COVID-19 SIR model Malaysia, fitted and observed", name),
...
fitplot1_log = ggplot(fitted_projected, aes(x = Date)) + geom_line(aes(y = I, color = "Infectious")) + 
  ...
  labs(y = "Infectious", title = paste("COVID-19 SIR model Malaysia, fitted and observed,", name, "log10"),
       subtitle = paste("Projection from data:", start_date, "to", fitted_projected$Date[max(Day)]),
       color = paste0("R square 1 = ", round(R2_1,3), "\n",
                      "R square 2 = ", round(R2_2,3), "\n",
                      "R0 = ", round(R0, 3), "\n",
...
fitted_projected[fitted_projected$Date == fitted_projected$Date[max(Day)], ]
fitted_projected[fitted_projected$Date == today(), ]
new_today = (fitted_projected[fitted_projected$Date == today(), ] - fitted_projected[fitted_projected$Date == today()-1, ])$total_infected
new_today
(fitted_projected[fitted_projected$Date == today()+1, ] - fitted_projected[fitted_projected$Date == today(), ])$total_infected
